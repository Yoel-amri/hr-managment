...
export default function InvitationsHistory({ me, updatedInvites }) {
  ...
  const cancelInvite = (user_id) => {
    let roleUrl = "admins";
    if (me.role === "SYSTEM_ADMIN") roleUrl = "system_admin";
    axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/${roleUrl}/employee/cancelInvite`, {
        user_id: user_id,
    }, {
        withCredentials: true,
    }).then((res) => {
    ...
    .catch((err) => {
        setSeverity('error');
        setAlertMessage('Error cancel invite!');
        setOpen(true);
    })
  };
...
const  getSysAdmins= () =>{
  ...
  return (<div>
      {me.role === 'SYSTEM_ADMIN' && <FormControlLabel control={<Switch showSysAdmin={showSysAdmin} onChange={getSysAdmins} defaultChecked />} label="Only show system admins." ></FormControlLabel> }
                    ...
                    <TableCell align="right">
                      {
                        user.invitation === 'WAITING' && (
                            ...
                            color="error"
                          variant="outlined"
                          onClick={() => {cancelInvite(user.user_id)}}
                        >
                          Cancel
                        </Button>
                        )
